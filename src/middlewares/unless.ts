...
export type Path =
  | { methods?: string[]; path: string | RegExp; exclude?: string[] }
  | string
  | RegExp;
...
const matches = (cfg: Path, method: string, pathname: string): boolean => {
  ...
  if (_.isObject(cfg)) {
    const m =
      (cfg as any).methods.length !== 0
        ? (cfg as any).methods.includes(method)
        : true;
...
export default (paths: Path[] = []) => (mw: Middleware): Middleware => (
  ctx,
  next
) => {
